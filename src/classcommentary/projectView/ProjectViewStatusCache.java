 ...
 * $Id$
 */
package classcommentary.projectView;
...
public class ProjectViewStatusCache implements Disposable {
    ...
    public ProjectViewStatus get(VirtualFile file) {
        if (myActive.get()) {
            ...
            if (status != null && LOG.isDebugEnabled()) {
                PluginManager.getLogger().warn("[" + SEQ.get() + "] Found cached status for " + file.getPath() + ", " + getCacheReport() + ", status=" + status);
            }
            return status;
        }
        return null;
    ...
    public PutResult add(VirtualFile file, ProjectViewStatus candidate) {
        if (myActive.get()) {
            ...
            if (LOG.isDebugEnabled()) {
                PluginManager.getLogger().warn("[" + SEQ.get() + "] Cached candidate for " + file.getPath() +
    ...
    private boolean evictPath(String filePath) {
        ...
        if (result && LOG.isDebugEnabled()) {
            PluginManager.getLogger().warn("[" + SEQ.get() + "] Evicted status for " + filePath + ", sizeAfter=[" + getCacheReport() + "], evicted=" + oldStatus);
        }
        return result;
    }
    ...
    public boolean evictAll(Collection<VirtualFile> files) {
        if (myActive.get()) {
            ...
            if (LOG.isDebugEnabled()) {
                PluginManager.getLogger().warn("[" + SEQ.get() + "] Evicted bulk, totalCount=" + evictedCount + ", sizeAfter=[" + getCacheReport() + "]");
            }
            return result;
        }
        return false;
    ...
    private void disposeCache(String name, Map<?, ?> cache) {
        ...
        cache.clear();
        if (LOG.isDebugEnabled()) {
            PluginManager.getLogger().warn("[" + SEQ.get() + "] " + name + " cache disposed, had " + size + " entries");
        }
    }
